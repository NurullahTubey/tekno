...
def q_tablosunu_json_yazdir(q_tablosu):
    """Q-tablosunu JSON dosyasına kaydeder."""
    boyut = q_tablosu.shape[0]
    q_tablo_json = {}
    for i in range(boyut):
        for j in range(boyut):
            durum = f"({i}, {j})"
            q_tablo_json[durum] = {
                f"Eylem {eylem}": q_tablosu[i, j, eylem].item() for eylem in range(4)
            }
    with open("kayit.json", "w") as f:
        json.dump(q_tablo_json, f, indent=4)
...
def q_ogrenme_simulasyonu(hiperparametreler):
    ...
    # Q tablosunu JSON dosyasına kaydet
    q_tablosunu_json_yazdir(q_tablosu)
